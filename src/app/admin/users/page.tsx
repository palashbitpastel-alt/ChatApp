...
export default function AdminUsersPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Search Input */}
      <div className="bg-[#11141b] border border-[#262b36] rounded-xl p-3">
      ...
      {/* Users Table */}
      <div className="bg-[#11141b] border border-[#262b36] rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-[#8b93a7]">
            <thead className="bg-[#1a1e27]/70 text-[#e8eaf0] uppercase text-[11px] font-semibold border-b border-[#262b36]">
              <tr>
            ...
            <tbody className="divide-y divide-[#262b36]/60">
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-10 text-xs text-[#8b93a7]">
                ...
                users.map((u) => (
                  <tr key={u.id} className="hover:bg-[#1a1e27]/30 transition-colors">
                    {/* User Info */}
                    <td className="px-5 py-3.5 flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm text-[#e8eaf0] flex items-center space-x-1.5">
                          <span>{u.username}</span>
                          {u.isOnline && (
                            <span className="w-2 h-2 rounded-full bg-[#6366f1]" title="Online" />
                          )}
                        </p>
                        <p className="text-[11px] text-[#8b93a7] truncate max-w-xs">
                          {u.statusMessage || "—"}
                        </p>
                      </div>
                    </td>
                    ...
                    {/* Phone */}
                    <td className="px-4 py-3.5 font-mono text-[#e8eaf0]">{u.phone}</td>

                    {/* Role */}
                    <td className="px-4 py-3.5">
                      {u.role === "ADMIN" ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-[#6366f1]/20 text-[#818cf8] border border-[#6366f1]/30">
                          <Shield className="w-3 h-3 mr-1" />
                          ADMIN
                        </span>
                      ) : (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-[#1a1e27] text-[#8b93a7]">
                          USER
                        </span>
                      )}
                    </td>
                    ...
                    {/* Message Count */}
                    <td className="px-4 py-3.5 text-[#e8eaf0] font-medium">
                      {u._count.messages}
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-[11px]">
                      {u.isOnline ? (
                        <span className="text-[#818cf8] font-medium">Online now</span>
                      ) : (
                    ...
                    <td className="px-5 py-3.5 text-right space-x-1.5">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded text-xs bg-[#1a1e27] hover:bg-[#232835] text-[#e8eaf0] border border-[#262b36] transition-colors"
                      ...
                      <button
                        ...
                        className="p-1 text-[#8b93a7] hover:text-rose-400 hover:bg-rose-500/10 rounded transition-colors inline-block"
                        title="Delete user"
                      >
